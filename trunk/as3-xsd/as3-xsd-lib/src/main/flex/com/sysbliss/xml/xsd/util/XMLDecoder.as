...
//      else
        ...
        for each (var childDefinition:XML in allElements)
        {
            ...
            {
                // <element>
                if (!decodeGroupElement(childDefinition, parent, valueElements, context, requireChild, hasSiblings,allElements.length()))
                    break;
            }
                                ...
                                context:DecodingContext = null, isRequired:Boolean = true):Boolean
    {
        ...
        for (choiceOccurs = 0; choiceOccurs < maxOccurs; choiceOccurs++)
        {
            ...
            var idx:int = 0;
            for(idx = 0;idx<choiceElements.length();idx++)
            //for each (var childDefinition:XML in choiceElements)
            {
                var childDefinition:XML = choiceElements[idx];
                if (childDefinition.name() == constants.elementTypeQName)
                {
                    // <element>
                    var madeChoice:Boolean = decodeGroupElement(childDefinition, parent, valueElements, context, false, (choiceElements.length()>1), choiceElements.length());
                    if(madeChoice) {
                        choiceSatisfied = madeChoice;
                    }
    ...
    public function decodeGroupElement(definition:XML, parent:*, valueElements:XMLList,
                                context:DecodingContext = null, isRequired:Boolean = true, hasSiblings:Boolean = true, groupLength:int=0):Boolean
    {
        if (context == null)
        	context = new DecodingContext();
        ...
        if (maxOccurs > 1)
        {
            ...
            if (hasSiblings)
            {
                setValue(parent, elementQName, emptyArray, typeQName);
                         ...
                         && parent.object_proxy::content != undefined
                         && parent.object_proxy::content.hasOwnProperty(elementName)
                         && TypeUtils.isAccessorIterable(parent.object_proxy::content,elementName))
            {
               setValue(parent, elementQName, emptyArray, typeQName);
    ...
    public function decodeElementTopLevel(definition:XML, elementQName:QName, value:*):*
    {
        ...
        else if (definition.hasComplexContent())
        {
            ...
            var myQName:QName = null;
            if(elementQName != null) {
                myQName = elementQName;
            }
            if(typeQName != null) {
                myQName = typeQName;
            }
            content = createContent(myQName);
                                    ...
                                    context:DecodingContext = null, isRequired:Boolean = true):Boolean
    {
        ...
        for (sequenceOccurs = 0; sequenceOccurs < maxOccurs; sequenceOccurs++)
        {
            ...
            for each (var childDefinition:XML in sequenceElements)
            {
                ...
                {
                    // <element>
                    if (!decodeGroupElement(childDefinition, parent, valueElements, context, requireChild, hasSiblings, sequenceElements.length()))
                        break;
                }
                else if (childDefinition.name() == constants.groupQName)
                {
    ...
    public function decodeSimpleContent(definition:XML, parent:*, name:QName, value:*, restriction:XML = null):void
    {
        ...
        var parentValueName:QName = name;

        if (parent is ContentProxy) {
            if(ContentProxy(parent).object_proxy::isSimple == false) {
                parentValueName = new QName("","value");
            } else {
        ...
        if (childDefinition != null)
        {
            ...
            if (childDefinition.name() == constants.extensionQName)
            {
                ...
                if (isBuiltInType(baseType))
                {
                    ...
                    setSimpleValue(parent, parentValueName, result, baseType);
                }
                else
                {
    ...
    */
    ...
    public function setValue(parent:*, name:*, value:*, type:Object=null):void
    {
        if (parent != null)
        {
            ...
            //if the property on the parent is an array, push the value
            else if(parent is ContentProxy
                         && parent.object_proxy::content != undefined
                         && propertyName != null
                         && parent.object_proxy::content.hasOwnProperty(propertyName)
                         && TypeIterator.isIterable(parent.object_proxy::content[propertyName]))
            {
                if(!TypeIterator.isIterable(value)) {
                    TypeIterator.push(parent[name], value);
                } else {
                    var it:TypeIterator = new TypeIterator(value);
                    while(it.hasNext()) {
                        TypeIterator.push(parent[name], it.next());
                    }
                }
            }
            else if (propertyName != null)
            {
                ...
                try
                {
                    ...
                    else if(parent is ContentProxy && ContentProxy(parent).object_proxy::content.hasOwnProperty(propertyName) && (value is Array)) {
                        if(TypeUtils.isArray(ContentProxy(parent).object_proxy::content,propertyName)) {
                            ContentProxy(parent).object_proxy::content[propertyName] = value;
                        } else if(TypeUtils.isArrayCollection(ContentProxy(parent).object_proxy::content,propertyName)) {
                            ContentProxy(parent).object_proxy::content[propertyName] = new ArrayCollection(value);
                        } else if(TypeUtils.isXMLList(ContentProxy(parent).object_proxy::content,propertyName)) {
                            ContentProxy(parent).object_proxy::content[propertyName] = new XMLList();
                        }
